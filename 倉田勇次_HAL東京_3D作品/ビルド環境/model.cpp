...
void MakeVertexLists(char* FileName, std::vector<POLYGON>* polygonList)
{
	...
	for (int i = 0; i < model.IndexNum / 3; i++)
	{
		int vertex0 = model.IndexArray[i];
		int vertex1 = model.IndexArray[i + 1];
		int vertex2 = model.IndexArray[i + 2];

		POLYGON polygon;

		polygon.vertex[0] = model.VertexArray[vertex0];
		polygon.vertex[1] = model.VertexArray[vertex1];
		polygon.vertex[2] = model.VertexArray[vertex2];
...
// 呼び出し例
// MakeVertexLists(MODEL_PATH, &colliderPolygons);
